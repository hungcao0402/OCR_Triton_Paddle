...
from rec_preprocess import RecPreprocessor
...
class TritonPythonModel:
    def initialize(self, args):
        ...
        self.rec_preprocessor = RecPreprocessor(max_imgW=1280)
    ...
    def execute(self, requests):
        ...
        for request in requests:
            det_output = pb_utils.get_input_tensor_by_name(request, self.input_names[0])
            ori_img = pb_utils.get_input_tensor_by_name(request, self.input_names[1])
            shape_list = pb_utils.get_input_tensor_by_name(request, self.input_names[2])
            ...
            list_crop_img = self.rec_preprocessor(img_raw, dt_boxes)
